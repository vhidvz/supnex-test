...
import { Check, HealthCheckOptions } from './types';
...
export class HealthService {
  ...
  public check() {
    ...
    ['disk', 'memory', 'mongo', 'redis', 'micro', 'kafka', 'grpc'].forEach(
      (check: Check) => {
        ...
        if (exists) {
          if (typeof exists === 'string') {
            ...
            if (['grpc', 'micro'].includes(check)) {
              ...
              indicators.push(
                this[`${check}Status`](exists.key, exists.options as never),
              );
            }
          }
        }
  ...
  protected memoryStatus(key = 'memory', options = 150 * 1024 * 1024) {
    return [
  ...
  protected diskStatus(
    key = 'disk',
    options = { thresholdPercent: 0.75, path: '/' },
  ) {
